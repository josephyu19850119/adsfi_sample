...
bool MdcYamlNode::Init()
{
    if (configFileName.empty()) {
        std::cout << "The configuration file name is empty.";
        return false;
    } 
    fh = fopen(configFileName.c_str(), "rb");
    if (fh == NULL) {
        std::cout << "Failed to open file! file name is "<< configFileName.c_str();
        return false;
    }
    return true;
}
...
bool MdcYamlNode::LoadYamlFile()
{
    ...
    do {
        if (!yaml_parser_parse(&parser, &event)) {
            std::cout << "Parser error is "<< parser.error;
            return false;
        }
        MdcYamlNode mapObj;
